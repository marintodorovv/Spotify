...
class ShowPlaylistCommandTest {
    ...
    void setUp() {
        ...
        command = new ShowPlaylistCommand(playlistService);
    ...
    void testExecuteSuccess() throws SpotifyException {
        ...
        when(request.getArgument(CommandArgument.PLAYLIST_NAME)).thenReturn("playlist");
        when(playlistService.getAllSongsFromPlaylist("playlist", "test@abv.bg"))
                .thenReturn(List.of("Star67"));
    ...
    @Test
    void testExecuteEmptyPlaylist() throws SpotifyException {
        User user = new User("test@abv.bg", "password");
        userContext.login(user);
        when(request.getArgument(CommandArgument.PLAYLIST_NAME)).thenReturn("playlist");
        when(playlistService.getAllSongsFromPlaylist("playlist", "test@abv.bg"))
